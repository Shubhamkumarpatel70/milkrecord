...
export default function AnalyticsPage({
  ...
  const chartData = [
    { month: 'Jul', milk: 1200, revenue: 60000, customers: Math.max(1, totalCustomers - 5) },
    { month: 'Aug', milk: 1500, revenue: 75000, customers: Math.max(2, totalCustomers - 4) },
    { month: 'Sep', milk: 1800, revenue: 90000, customers: Math.max(3, totalCustomers - 3) },
    { month: 'Oct', milk: 2100, revenue: 105000, customers: Math.max(4, totalCustomers - 2) },
    { month: 'Nov', milk: 2400, revenue: 120000, customers: Math.max(5, totalCustomers - 1) },
    { month: 'Dec', milk: totalMilkQuantityKg > 0 ? totalMilkQuantityKg : 2700, revenue: totalRevenue > 0 ? totalRevenue : 135000, customers: totalCustomers > 0 ? totalCustomers : 6 },
  ];

  const handleExportExcel = () => {
    const dataToExport = (customers.length > 0 ? customers : [
      { name: 'Ramesh Patel', whatsapp: '9876543210', month: '2024-01', totalDays: 15, totalAmount: 4500, paidAmount: 3000, status: 'unpaid' }
    ]).map(c => ({
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in pb-8 w-full max-w-full overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-soft">
        <div className="flex items-center gap-3">
          <div className="p-2.5 sm:p-3 rounded-2xl bg-purple-100 dark:bg-purple-950 text-purple-600 dark:text-purple-400 shrink-0">
            <BarChart3 className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100 leading-tight">Analytics & Intelligence</h1>
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs shadow-md flex items-center justify-center gap-2 transition-colors min-h-[44px]"
          >
          ...
          <button
            onClick={handlePrint}
            className="btn-secondary text-xs py-2.5 px-4 flex items-center justify-center gap-2 min-h-[44px] shrink-0"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="p-4 sm:p-6 rounded-3xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-soft">
